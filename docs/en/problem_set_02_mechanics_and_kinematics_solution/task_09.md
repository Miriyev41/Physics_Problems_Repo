...
3. Create an animation with two panels:
   ...
   - panel B: trajectory of Mars as seen from Earth (trace of $\vec{r}_{M/Z}(t)$).
...
&= \begin{pmatrix} R_M \cos(\omega_M t) - R_Z \cos(\omega_Z t) \\ R_M \sin(\omega_M t) - R_Z \sin(\omega_Z t) \end{pmatrix}
\end{aligned}
$$
...
* **Heliocentric equations:** $\vec{r}_Z(t) = (R_Z \cos(\omega_Z t), R_Z \sin(\omega_Z t))$ and $\vec{r}_M(t) = (R_M \cos(\omega_M t), R_M \sin(\omega_M t))$
